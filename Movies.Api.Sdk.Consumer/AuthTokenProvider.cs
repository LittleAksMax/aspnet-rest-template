...
public class AuthTokenProvider
{
    ...
    // so only 1 request can be sent at any time
    ...
    public async Task<string> GetTokenAsync(HttpRequestMessage _, CancellationToken token)
    {
        if (!string.IsNullOrEmpty(_cachedToken))
        {
            ...
            var expiryDateTime = UnixTimeToDateTime(int.Parse(expiryTimeText));
        ...
        // release lock on this segment
        Lock.Release();
        
        return _cachedToken;
    }

    private static DateTime UnixTimeToDateTime(int unixTimeStamp)
    {
